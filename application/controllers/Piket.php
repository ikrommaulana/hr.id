...
class Piket extends CI_Controller {
    ...
    function report($id_piket){
        ...
        $data_karyawan = $this->db->query("SELECT employee.*, division.init_division_name, jabatan.nama_jabatan FROM employee,division,jabatan WHERE employee.id_employee='$id_employee' and employee.id_division = division.id_division and employee.id_position = jabatan.id_jabatan")->result();
        $data_approver = $this->db->query("SELECT  employee.*, division.init_division_name, jabatan.nama_jabatan FROM employee,division,jabatan WHERE employee.id_employee='$id_approver' and employee.id_division = division.id_division and employee.id_position = jabatan.id_jabatan")->result();
                            ...
                            <tr>
                                <td valign='top' colspan='3' style='padding-left:20px'>
                                    <p style='font-family:Verdana,Geneva,Sans-serif;''>
                                        ...
                                        ".$data_approver[0]->Title." ".$data_approver[0]->firstname."
                                    </p> 
                                    <p>".$data_karyawan[0]->title." ".$data_karyawan[0]->firstname." mengajukan laporan hasil piket : </p>
                                </td>
                            </tr>
                            <tr>
    ...
    public function add_edit_data($id_piket=''){
        ...
        for($a=0;$a<=$interval;$a++){
            $tanggal_libur  = date('Y-m-d', strtotime('+'.$a.' days', strtotime($date[0])));
            $hari           = date('l', strtotime('+'.$a.' days', strtotime($date[0])));
            $cek_libur = $this->db->query("SELECT * FROM hari_libur WHERE tanggal='$tanggal_libur'")->result();
                                    ...
                                    <table align='center' width='800px' cellpadding='0' cellspacing='1' bgcolor='#ffffff' border='0' style='font-family:Verdana,Geneva,Sans-serif;'>
                                        ...
                                        <tr>
                                            <td valign='top' colspan='3' style='padding-left:20px'>
                                                ...
                                                Kepada Yth, <br>
                                                ".$data_approver[0]->nama_jabatan." ".$data_approver[0]->init_division_name."<br>
                                                ".$data_approver[0]->Tile." ".$data_approver[0]->firstname."
                                    ...
                                    <table align='center' width='800px' cellpadding='0' cellspacing='1' bgcolor='#ffffff' border='0' style='font-family:Verdana,Geneva,Sans-serif;'>
                                        ...
                                        <tr>
                                            <td valign='top' colspan='3' style='padding-left:20px'>
                                                ...
                                                <p>".$data_approver[0]->title." ".$data_approver[0]->firstname." memerintahkan Anda piket :  </p>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td valign='top' style='padding-left:20px'>
    ...
    function hitung_ulang($id_piket=''){
        $this->session->set_flashdata("halaman", "daftar"); //mensetting menuKepilih atau menu aktif
        $user_id = $this->session->userdata('user_id');
        $id_jabatan = $this->session->userdata('id_jabatan');
        $id_division = $this->session->userdata('id_division');
        ...
        if($id_piket){
            $cek_piket  = $this->db->query("SELECT * FROM piket WHERE id_piket='$id_piket'")->result();
            $start      = $cek_piket[0]->start_date;
            $end        = $cek_piket[0]->end_date;
