                                                            ...
                                                            <form class="floating-labels" method="post" action="<?=base_url()?>biaya/add_edit_data/<?=$id_biaya_dinas;?>">
                                                              ...
                                                              <div class="row">
                                                                <?php foreach($data_jabatan as $view){
                                                                    $biaya = $this->db->query("SELECT * FROM biaya_dinas_detail2 WHERE id_jabatan='$view->id_jabatan' AND id_biaya_dinas='$id_biaya_dinas'")->result();
                                                                    $value = (empty($biaya)) ? 0 : $biaya[0]->biaya;
                                                                ?>
                                                                  <div class="form-group m-b-40 col-sm-2">
                                                                      <input type="int" class="form-control" id="input<?=$view->id_jabatan;?>" name="<?=$view->id_jabatan;?>" value="<?=$value;?>" required><span class="highlight"></span> <span class="bar"></span>
